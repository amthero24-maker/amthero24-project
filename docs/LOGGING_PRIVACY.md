...
Long text is truncated after redaction. Binary values are replaced with their byte length only.
...
The policy scans runtime Python modules, excluding `tests/` and `scripts/`, and rejects:

- runtime `print()` calls
- direct sensitive identifiers passed to logger methods
- sensitive f-string or concatenated logging messages
- sensitive `extra` mappings

The report contains only file, line, rule, and identifier. It does not read runtime values.

## Automated workflow

The `Log Safety` workflow runs on relevant pull requests, pushes to `main`, weekly schedules, and manually. It:

1. validates the static logging policy
2. tests string, structured-value, exception, and global-factory redaction
3. uploads sanitized test and policy evidence
4. fails if either policy or tests fail
