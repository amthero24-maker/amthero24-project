    ...
    try:
        ...
        with store.pool.connection() as connection:
            row = connection.execute(
                "SELECT version, name, checksum, app_version FROM amthero_schema_migrations"
            ).fetchone()
...
def test_two_replicas_serialize_the_first_migration(isolated_schema: str) -> None:
    ...
    try:
        ...
        assert sum(len(report.applied_versions) for report in reports) == 1
        with first_store.pool.connection() as connection:
            count = connection.execute(
                "SELECT COUNT(*) AS count FROM amthero_schema_migrations WHERE version = 1"
            ).fetchone()
