...
const required = ['LEGAL_OPERATOR_NAME','LEGAL_TRADING_NAME','LEGAL_COUNTRY','LEGAL_EMAIL_INFO','LEGAL_EMAIL_SUPPORT'];
const missing = required.filter((key) => !process.env[key] || /\[.*\]/.test(process.env[key]));
if (publicRelease) {
  const approvals = ['LEGAL_PROVIDER_DPAS_CONFIRMED','LEGAL_META_TERMS_CONFIRMED','LEGAL_RAILWAY_REGION_CONFIRMED','LEGAL_WEBSITE_REVIEW_CONFIRMED','DOMAIN_PRIMARY_TLS_CONFIRMED','DOMAIN_REDIRECTS_CONFIRMED','EMAIL_INBOUND_ROUTING_CONFIRMED','EMAIL_OUTBOUND_AUTH_CONFIRMED','AI_TRANSPARENCY_REVIEW_CONFIRMED'];
